...
**補足: pushのたびにPAT入力が必要な理由と対策**

5-1で `credential.helper ""` に設定していると、`git push` / `git pull` のたびにユーザー名とPATの入力が必要になる。授業時間内だけ入力を省略したい場合は、OS側の永続的な資格情報マネージャーには保存せず一定時間だけメモリ上にキャッシュする方法もある。

```bash
git config credential.helper "cache --timeout=3600"
```

最初の1回だけPATを入力すれば、指定時間(この例では3600秒=1時間)は再入力不要になる。時間経過後や再起動後は自動的に消えるため、共有PCでも比較的安全に使える。

任意にキャッシュを削除する場合は次を実行する。

```bash
git credential-cache exit
```

これでキャッシュしていた認証情報がその場で破棄される。共有PCを離れる前は、時間任せにせずこのコマンドで明示的に消しておくとより安全。
...
実習室・貸出PCがWindowsの場合に特に気をつけたい点をまとめる。

- **Git Credential Managerが既定で有効なことが多い**: Windows版Gitには「Git Credential Manager(GCM)」が標準で入っており、`credential.helper` を明示的に空にしない限り、認証情報がWindowsの資格情報マネージャーに自動保存される。5-1(1)の設定を必ず行うこと。
- **資格情報マネージャーの確認・削除方法**: コントロールパネル → ユーザーアカウント → 資格情報マネージャー(Credential Manager)の「Windows資格情報」タブから `git:https://github.com` を検索・削除できる。コマンドでも確認・削除が可能。
...
- **ssh-agentサービスに注意**: Windows 11にはSSH鍵をキャッシュする「OpenSSH Authentication Agent」サービスがある。有効な状態で `ssh-add` すると、PCを再起動するまで次の受講者からも鍵が使える状態になる。共有PCでは `ssh-add` で鍵を登録しない。万一登録した場合は `ssh-add -D` で全て削除する。
- **作業フォルダはOneDrive同期の対象外に置く**: 「デスクトップ」「ドキュメント」は既定でOneDriveと同期されていることが多く、リポジトリやSSH鍵を置くと意図せずクラウドに残ったり、同期処理とGitの操作が競合したりする。`C:\work\` のようにドライブ直下にフォルダを作る場合は管理者権限が必要になることがあるため、`C:\Users\ユーザー名\work\` のように自分のユーザープロファイル配下(かつDesktop/Documents以外)を使うと、管理者権限なしでOneDrive同期も避けられる。
- **隠しフォルダが見えないことがある**: `.sshkey` のような `.` から始まるフォルダは、エクスプローラーの設定によって表示されない場合がある。「表示」→「隠しファイル」にチェックを入れるか、PowerShellで `Get-ChildItem -Force` を使って削除漏れがないか確認する。
- **PowerShellのスクリプト実行制限**: 実行ポリシーにより `.ps1` スクリプトが実行できないことがある。共有PCでは学生権限でポリシーを変更できないことが多いため、Git BashまたはGitのコマンドを直接使う。
